            ...
            if slimming is True and map_update_count % SLIMMING_PERIOD == 0:
                ...
                for ty in range(0, MAP_SIZE, TILE_SIZE):
                    for tx in range(0, MAP_SIZE, TILE_SIZE):
                        ...
                        # ================= entropy → slimming policy =================
                        if H < 1.1:              # STABLE
                            dist_thr = 0.8       # 강한 slimming
                            slim_factor = 0.6
                        elif H < 1.6:            # MID
                            dist_thr = 1.2       # 보수적 slimming
                            slim_factor = 0.75
                        else:                    # CHAOTIC
                            continue             # 아직 불안정 → slimming 자체 금지
                        ...
                        # ================= seed 보호 =================
                        seed_set = set(map(tuple, pts.astype(int)))
                        ...
                        reinforce_dist = dist_thr * 0.4   # line에 매우 가까운 점만
                        reinforce_gain = 1.05             # 아주 약하게
                        sigma = dist_thr * 0.5     # line 두께의 절반
                        R = max(1, int(3 * sigma))
                        splat_gain = 0.03          # 누적형 (작게!)
                        ...
                        for (x, y), d in zip(all_pts.astype(int), dist_all):
                            ...
                            # --- reinforce + splat ---
                            if d < reinforce_dist and reinforcement is True:

                                # 1️⃣ seed 점 자체 보강
                                tile[y, x] = min(tile[y, x] * reinforce_gain, 1.0)

                                # 2️⃣ gaussian splatting
